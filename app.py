...
from location import (
    ...
    get_nearest_volunteers_with_postgis,
...
def find_nearest_volunteers():
    if request.is_json:
        ...
        # Call your new, fast function instead of the old one.
        volunteers = get_nearest_volunteers_with_postgis(
            lat=lat, 
            lon=lon,
            # You can still pass in other optional parameters from the request
            limit=data.get('limit', config.DEFAULT_SEARCH_LIMIT),
            min_radius_km=data.get('min_radius', config.MIN_SEARCH_RADIUS_KM),
            max_radius_km=data.get('max_radius', config.MAX_SEARCH_RADIUS_KM),
            exception_id=set(data.get('exception_id', []))
        )
